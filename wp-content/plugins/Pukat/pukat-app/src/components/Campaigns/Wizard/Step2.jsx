...
import { PLAYBOOKS } from './wizardData.js'

export default function Step2({ form, setForm, onBack, onNext }) {
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {PLAYBOOKS.map(pb => (
            <button
              ...
              aria-pressed={form.playbook === pb.id}
              className={clsx(
                'rounded-xl p-4 cursor-pointer select-none text-left transition-all',
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
        ...
        <button type="button" onClick={onNext} className="bg-violet-500 text-white hover:bg-violet-600 px-5 py-2 text-sm font-semibold rounded-xl transition-all">
          Continue to review →
        </button>
      </div>
    </div>
